 ...
 *
 */
var SceneGame_YSC = (function (_super) {
    __extends(SceneGame_YSC, _super);
    //定义界面
    function SceneGame_YSC() {
        var _this = _super.call(this) || this;
        //定义界面 
        _this.skinName = SceneGame_YSCSkin;
        //显示界面
        ...
        basic.Dispatcher.addListener(EventNames.YSC_GAMEOPEN, _this.onGameOpen, _this);
        basic.Dispatcher.addListener(EventNames.YSC_GAMEINFO, _this.onGameInfo, _this);
        basic.Dispatcher.addListener(EventNames.YSC_GAMERESULT, _this.onShowGameResult, _this);
        basic.Dispatcher.addListener(EventNames.YSC_CHANGESTATUS, _this.onChangeStatus, _this);
        return _this;
    }
    //显示前调用
    SceneGame_YSC.prototype.beforeShow = function () {
        //初始化界面
        this.play.playLight();
    ...
    //隐藏时调用
    SceneGame_YSC.prototype.onHide = function () {
        //清除界面
        this.chipfly.clean();
        this.play.stopLight();
        //数据赋值
    ...
    //退出按钮
    SceneGame_YSC.prototype.onExitBtn = function (e) {
        //播放声音
    ...
    //-------------------------定义事件-------------------------
    //游戏初始化
    SceneGame_YSC.prototype.onGameInfo = function (e) {
        //判断显示界面显示
        if (e.data.status >= 2) {
            this.yazhu.alpha = 0;
        ...
        for (var i1 = 0; i1 < 4; i1++) {
            for (var j1 = 0; j1 < e.data.totalBetGoldDetails[i1].length; j1++) {
        ...
        for (var i2 = 0; i2 < 4; i2++) {
            for (var j2 = 0; j2 < e.data.totalGrabGoldDetails[i2].length; j2++) {
    ...
    //改变状态事件
    SceneGame_YSC.prototype.onChangeStatus = function (e) {
        //判断显示
        if (e.data.status == 0) {
            //清除界面
            this.play.clean();
    ...
    //用户结果
    SceneGame_YSC.prototype.onShowGameResult = function (e) {
        //数据赋值
        UserData.User_Gold = e.data.gold;
    };
    //游戏开始
    SceneGame_YSC.prototype.onGameOpen = function (e) {
        //最后核对数据
        this.yazhu.checkData(e.data);
        //显示界面
        var _tween_alpha1 = egret.Tween.get(this.yazhu).to({ alpha: 0 }, 600);
        var _tween_alpha2 = egret.Tween.get(this.chipfly).to({ alpha: 0 }, 600);
        //开始游戏
        GameData.YSC_RUN_OVER = e.data.pos;
        this.play.startPlay(e.data.pos, 600);
    };
    //金币改变
    SceneGame_YSC.prototype.onGoldChange = function (e) {
        //显示总金币
    ...
    //---------------------------显示位置------------------------
    SceneGame_YSC.prototype.onShowPlace = function () {
        //定义变量
        var rate;
        var zoom;
        //判断显示
        ...
        this.chipfly.info(this.play.scaleX, this.play.x, this.play.y, this.choose.x, this.choose.y, this.usergold.x, this.usergold.y);
    };
    return SceneGame_YSC;
}(basic.SceneBase));
__reflect(SceneGame_YSC.prototype, "SceneGame_YSC");
//# sourceMappingURL=SceneGame_YSC.js.map