 ...
 *
 */
var SceneGame_NN = (function (_super) {
    __extends(SceneGame_NN, _super);
    //定义界面
    function SceneGame_NN() {
        ...
        //定义界面
        _this.skinName = SceneGame_NNSkin;
        //数据赋值
        ...
        basic.Dispatcher.addListener(EventNames.NN_ERROR, _this.onShowTips, _this);
        basic.Dispatcher.addListener(EventNames.NN_SHOWGOLD, _this.onShowGold, _this);
        basic.Dispatcher.addListener(EventNames.NN_GAMEOPEN, _this.onGameOpen, _this);
        basic.Dispatcher.addListener(EventNames.NN_GAMEINFO, _this.onGameInfo, _this);
        basic.Dispatcher.addListener(EventNames.NN_ENTERROOM, _this.onEnterRoom, _this);
        basic.Dispatcher.addListener(EventNames.NN_GAMERESULT, _this.onGameResult, _this);
        basic.Dispatcher.addListener(EventNames.NN_CHANGEZHUANG, _this.onChangeZhuang, _this);
        basic.Dispatcher.addListener(EventNames.NN_CHANGESTATUS, _this.onChangeStatus, _this);
        basic.Dispatcher.addListener(EventNames.NN_SEND_CARDOVER, _this.onSendCardOver, _this);
        return _this;
        //this.clock.startTimer(1,6);
        //this.sendcard.start();
        //this.chipfly.infoChip(19);
    }
    //显示前调用
    SceneGame_NN.prototype.beforeShow = function () {
        //判断显示位置
        this.onShowPlace();
        //数据赋值
    ...
    //隐藏时调用
    SceneGame_NN.prototype.onHide = function () {
        //清除界面
        this.clean();
        //数据赋值
        GameData.Game_Id = 1;
        ...
        //发送消息
        //Comm_nn.instance.sendSocket({ "type": "exitGame" });
    };
    //清除界面
    SceneGame_NN.prototype.clean = function () {
        //清除界面
        this.zhuang.clean();
        this.chipfly.clean();
        ...
        //隐藏结算
        basic.Dispatcher.dispatch(EventNames.NN_HIDEOVER);
        //判断停止麻将
    ...
    //显示金币
    SceneGame_NN.prototype.showGold = function () {
        //定义变量
        var show_gold;
        //数据赋值
        show_gold = UserData.User_Gold - GameData.NN_YaZhu_UserTotal;
        //显示金币
        this.txt_gold_user.text = show_gold.toString();
    };
    //显示时钟
    SceneGame_NN.prototype.showClock = function (_time) {
        if (_time === void 0) { _time = 0; }
        //显示时钟
        if (GameData.NN_Game_Status == 0) {
            //显示倒计时
            this.clock.startTimer(GameData.NN_Game_Status, 0);
        }
        else if (GameData.NN_Game_Status == 1) {
            this.clock.startTimer(GameData.NN_Game_Status, _time - 2, function () {
                //数据赋值
                GameData.NN_Game_Status = 2;
            });
        }
        else {
            this.clock.startTimer(GameData.NN_Game_Status);
        }
    };
    //显示房间
    SceneGame_NN.prototype.showRoom = function () {
        //判断显示按钮
        if (GameData.Game_Room_Id == "0") {
    ...
    //-------------------------定义事件-------------------------
    //显示提示
    SceneGame_NN.prototype.onShowTips = function (e) {
        //显示提示
        basic.Dispatcher.dispatch(EventNames.SHOW_TIPS, { "tips": e.data.msg });
    };
    //金币改变
    SceneGame_NN.prototype.onGoldChange = function (e) {
        //显示金币
        this.showGold();
    };
    //显示金币
    SceneGame_NN.prototype.onShowGold = function (e) {
        //显示金币
        this.showGold();
    };
    //进入房间
    SceneGame_NN.prototype.onEnterRoom = function (e) {
        //数据赋值
    ...
    //游戏初始化
    SceneGame_NN.prototype.onGameInfo = function (e) {
        //定义变量
        var lefttime;
        //判断显示
        if (GameData.Game_Room_Id != "0") {
        ...
        //数据赋值
        GameData.Game_Room_Id = e.data.roomId;
        GameData.NN_Game_Status = e.data.status;
        ...
        GameData.NN_Poker_Table_Card = e.data.oldCards;
        GameData.NN_Poker_Table_NewCard = e.data.cards;
        GameData.NN_Poker_Table_IsNiu = e.data.niuCards;
        GameData.NN_Poker_Table_Type = e.data.cardTypes;
        GameData.NN_Poker_Table_IsWin = e.data.isWins;
        GameData.NN_UpZhuang_Condition = e.data.callDealerMinGold;
    ...
    //结算事件
    SceneGame_NN.prototype.onGameResult = function (e) {
        //数据赋值
        UserData.User_Gold = e.data.gold;
        GameData.Game_Zhuang_Gold = e.data.dealerTotalGold;
    };
    //状态改变事件
    SceneGame_NN.prototype.onChangeStatus = function (e) {
        //数据赋值
        GameData.NN_Game_Status = e.data.status;
        //显示时钟
        if (GameData.NN_Game_Status == 0) {
            ...
            //显示倒计时
            this.clock.startTimer(GameData.NN_Game_Status, e.data.leftTime);
            //隐藏结算
            basic.Dispatcher.dispatch(EventNames.NN_HIDEOVER);
        }
        else if (GameData.NN_Game_Status == 1) {
            //显示倒计时
            this.clock.startTimer(GameData.NN_Game_Status, e.data.leftTime - 2, function () {
                //数据赋值
                GameData.NN_Game_Status = 2;
            });
            //隐藏结算
            ...
            this.clock.startTimer(GameData.NN_Game_Status);
            //判断显示结果
            if (GameData.NN_Game_Status == 3) {
                //判断显示特效
                if (GameData.NN_Poker_Table_IsWin[0] == 1 && GameData.NN_Poker_Table_IsWin[1] == 1 && GameData.NN_Poker_Table_IsWin[2] == 1 && GameData.NN_Poker_Table_IsWin[3] == 1 && GameData.NN_Poker_Table_IsWin[4] == 1) {
                    //显示通赔
                    this.specialresult.showLose(function () {
                        //显示结算
                        basic.Dispatcher.dispatch(EventNames.NN_SHOWOVER);
                    });
                }
                else if (GameData.NN_Poker_Table_IsWin[0] == -1 && GameData.NN_Poker_Table_IsWin[1] == -1 && GameData.NN_Poker_Table_IsWin[2] == -1 && GameData.NN_Poker_Table_IsWin[3] == -1 && GameData.NN_Poker_Table_IsWin[4] == -1) {
                    //显示通杀
    ...
    //游戏开牌
    SceneGame_NN.prototype.onGameOpen = function (e) {
        //数据赋值
        GameData.NN_Poker_Table_Card = e.data.oldCards;
        GameData.NN_Poker_Table_NewCard = e.data.cards;
        GameData.NN_Poker_Table_Type = e.data.cardTypes;
        ...
        for (var i = 0; i < 6; i++) {
            basic.Dispatcher.dispatch(EventNames.NN_SHOW_CARDDETAIL, {
                "table": i,
                "card": GameData.NN_Poker_Table_Card[i]
            });
        }
        //开始动画
        this.sendcard.start();
    ...
    //改变庄事件
    SceneGame_NN.prototype.onChangeZhuang = function (e) {
        //判断显示提示
    ...
    //发送牌结束
    SceneGame_NN.prototype.onSendCardOver = function (e) {
        //显示牌
        basic.Dispatcher.dispatch(EventNames.NN_SHOW_CARD, { "table": -1, "cardnum": 0 });
        //数据赋值
    ...
    //停止打开牌
    SceneGame_NN.prototype.stopOpenCard = function () {
        //判断停止麻将
        if (this.timer_opencard) {
            this.timer_opencard.stop();
            this.timer_opencard.removeEventListener(basic.TimerEvent.TIMER, this.onOpenCard, this);
    ...
    //打开牌
    SceneGame_NN.prototype.onOpenCard = function (e) {
        //判断发送消息
        if (this.open_card_num < 6) {
            basic.Dispatcher.dispatch(EventNames.NN_OPEN_CARD, {
    ...
    //打开牌结束
    SceneGame_NN.prototype.onOpenCardComplete = function (e) {
        //停止打开牌
        this.stopOpenCard();
        //显示界面
        for (var i = 0; i < 5; i++) {
    ...
    //-------------------------定义按钮-------------------------
    //按钮事件
    SceneGame_NN.prototype.onExitBtn = function (e) {
        //播放声音
    ...
    //路子按钮
    SceneGame_NN.prototype.onLuZiBtn = function (e) {
        //播放声音
        basic.SoundManager.instance.playEffect("sound_btn_mp3");
        //显示界面
        Panel_NN_LuZi.instance.show();
    };
    //牌型按钮
    SceneGame_NN.prototype.onPaiXinBtn = function (e) {
        //播放声音
        basic.SoundManager.instance.playEffect("sound_btn_mp3");
        //显示界面
        Panel_NN_PaiXing.instance.show();
    };
    //选择头像
    SceneGame_NN.prototype.onChooseHead = function (e) {
        var _this = this;
        //显示选择头像
    ...
    //上庄按钮
    SceneGame_NN.prototype.onUpZhuangBtn = function (e) {
        //播放声音
        basic.SoundManager.instance.playEffect("sound_btn_mp3");
        //判断显示
        if (GameData.Game_Zhuang_Id == UserData.User_Id) {
            //发送消息
            Comm_nn.instance.sendSocket({ "type": "cancelDealer" });
        }
        else {
            //显示界面
            Panel_UpZhuang.instance.show(1);
        }
    };
    //选择筹码按钮
    SceneGame_NN.prototype.onChooseChipBtn = function (e) {
        var _this = this;
        //播放声音
        basic.SoundManager.instance.playEffect("sound_btn_mp3");
        //显示界面
    ...
    //桌子按钮
    SceneGame_NN.prototype.onTableBtn = function (e) {
        //定义变量
        var btnnum = Number(e.target.name);
        //判断显示压住
        if (GameData.NN_Game_Status == 1 && UserData.User_Id != GameData.Game_Zhuang_Id) {
            //判断显示
            if (GameData.NN_YaZhu_User[btnnum] + GameData.Game_Chip_Gold[GameData.Game_Chip_Now] > UserData.User_MaxYaZhu) {
                //显示提示
                basic.Dispatcher.dispatch(EventNames.SHOW_TIPS, { "tips": "超过每门可下金币！" });
            }
            else {
                if (GameData.Game_Room_Id != "0" && GameData.NN_YaZhu_User[btnnum] + GameData.Game_Chip_Gold[GameData.Game_Chip_Now] > 150000) {
                    //显示提示
                    basic.Dispatcher.dispatch(EventNames.SHOW_TIPS, { "tips": "超过私人房每门可下金币！" });
                }
                else {
                    //判断显示
                    if (GameData.NN_YaZhu_UserTotal + GameData.Game_Chip_Gold[GameData.Game_Chip_Now] <= UserData.User_Gold / 4) {
                        //数据赋值
                        GameData.NN_YaZhu_User[btnnum] += GameData.Game_Chip_Gold[GameData.Game_Chip_Now];
                        GameData.NN_YaZhu_User_Now[btnnum] += GameData.Game_Chip_Gold[GameData.Game_Chip_Now];
                        GameData.NN_YaZhu_UserTotal += GameData.Game_Chip_Gold[GameData.Game_Chip_Now];
                        //显示金币
                        this.showGold();
                        //显示界面
                        ...
                        //提交数据
                        Comm_nn.instance.sendSocket({ "type": "bet", "pos": btnnum, "gold": GameData.Game_Chip_Gold[GameData.Game_Chip_Now] });
                    }
                    else {
                        //显示提示
    ...
    //显示位置
    SceneGame_NN.prototype.onShowPlace = function () {
        //定义变量
        var proportion;
        var proportion_btn;
        ...
        send_to_x[0] = this.table[0].x + 100 - 64 * 0.75 - 123 * 0.65 / 2;
        send_to_x[1] = this.table[1].x + 100 - 64 * 0.75 - 123 * 0.65 / 2;
        send_to_x[3] = this.table[3].x + 100 - 64 * 0.75 - 123 * 0.65 / 2;
        send_to_x[4] = this.table[4].x + 100 - 64 * 0.75 - 123 * 0.65 / 2;
        ...
        send_to_x[5] = (basic.StageProxy.width - this.zhuang.width) / 2 + 79 - 64 - 123 * 0.65 / 2;
        send_to_x[2] = (basic.StageProxy.width - this.table[2].width) / 2 + 100 - 64 - 123 * 0.65 / 2;
        //麻将初始化
...
__reflect(SceneGame_NN.prototype, "SceneGame_NN");
//# sourceMappingURL=SceneGame_NN.js.map