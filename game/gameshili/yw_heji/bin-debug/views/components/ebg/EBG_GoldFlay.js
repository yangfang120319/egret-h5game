 ...
 *
 */
var EBG_GoldFlay = (function (_super) {
    __extends(EBG_GoldFlay, _super);
    function EBG_GoldFlay() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    //初始化
    EBG_GoldFlay.prototype.createChildren = function () {
        _super.prototype.createChildren.call(this);
    };
    //开始飞行
    EBG_GoldFlay.prototype.startFlay = function (_type, _direction, _table) {
        //定义变量
        var action_data = {};
        /**
        *
        ...
        */
        ...
        if (_type == 0) {
            //定义变量
            var place_user = this.assUserPlace();
            var place_table1 = this.assUserTablePlace(_table);
            //判断数据赋值
            if (_direction == 0) {
            ...
            //定义变量
            var place_zhuang = this.assZhuangPlace();
            var place_table2 = this.assTablePlace(_table);
            //数据复制
            if (_direction == 0) {
            ...
            //定义变量
            var place_online = this.assOnLinePlace();
            var place_table3 = this.assTablePlace(_table);
            //数据赋值
            if (_direction == 0) {
        ...
        //开始动画
        var flayaction = new GoldFlayAction();
        flayaction.startFlay(action_data);
    };
    //用户数据赋值
    EBG_GoldFlay.prototype.assUserPlace = function () {
        //定义变量
        var place = {};
        //数据赋值
        place["_x"] = 4;
    ...
    //庄数据赋值
    EBG_GoldFlay.prototype.assZhuangPlace = function () {
        //定义变量
        var place = {};
        //数据赋值
        place["_y"] = 80;
    ...
    //在线玩家复制
    EBG_GoldFlay.prototype.assOnLinePlace = function () {
        //定义变量
        var place = {};
        //数据赋值
        place["_x"] = 577;
    ...
    //桌子赋值
    EBG_GoldFlay.prototype.assUserTablePlace = function (_table) {
        //定义变量
        var place = {};
        //数据赋值
        if (_table == 0) {
    ...
    //桌子赋值
    EBG_GoldFlay.prototype.assTablePlace = function (_table) {
        //定义变量
        var place = {};
        //数据赋值
        if (_table == 0) {
...
__reflect(EBG_GoldFlay.prototype, "EBG_GoldFlay");
var GoldFlayAction = (function () {
    //初始化
    function GoldFlayAction() {
        this.timer_goldflay = null;
    }
    //开始飞行
    GoldFlayAction.prototype.startFlay = function (action_data) {
        //数据赋值
    ...
    //金币飞行中
    GoldFlayAction.prototype.onGoldFlay = function (e) {
        var _this = this;
        //定义变量
        var _to_x;
        var _to_y;
        ...
        //显示动画
        var _tween_flay_alpha = egret.Tween.get(img_gold).to({ alpha: 0.9 }, 50).wait(350).to({ alpha: 0 }, 50);
        var _tween_flay_x = egret.Tween.get(img_gold).to({ x: _to_x }, 450);
        var _tween_flay_y = egret.Tween.get(img_gold).to({ y: _to_y }, 450).call(function () {
            //移除图片
            _this.com_parent.removeChild(img_gold);
        });
    };
    //筹码发放结束
    GoldFlayAction.prototype.onGoldFlayComplete = function (e) {
        //停止等待
    ...
    return GoldFlayAction;
}());
__reflect(GoldFlayAction.prototype, "GoldFlayAction");
